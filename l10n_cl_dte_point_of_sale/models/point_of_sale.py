...
class POS(models.Model):
    ...
    def search_browse(self, domain):
        result = self.search(domain, order="id asc")
        logger.info(result)
    ...
    sequence_id = fields.Many2one(
            ...
            #default=lambda self: self.env["ir.sequence"].search([('sii_document_class_id.sii_code', '=', 39)]).id,
        )
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            pos_order.update({'document_class_id': self.env['sii.document_class'].search([('doc_code_prefix', '=', 'FAC')]).id})
    ...
    def do_dte_send(self, n_atencion=None):
        ...
        for id_class_doc, classes in clases.items():
            ...
            for documento in classes:
                #documento_envio = documento['envio']
                _logger.info(str(documento))
                #if documento['envio'] else ''
                documentos[id_class_doc] += '\n' + documento['envio']
                NroDte += 1
                if not str(id_class_doc) in file_name:
                    file_name.update({str(id_class_doc): ''})
                file_name[str(id_class_doc)] += 'F' + str(int(documento['sii_document_number'])) + 'T' + str(id_class_doc) + '.xml'
        ...
        for id_class_doc, documento in documentos.items():
            ...
            for order in self:
                if order.document_class_id.sii_code == id_class_doc:
                    order.sii_xml_request = envio_id
