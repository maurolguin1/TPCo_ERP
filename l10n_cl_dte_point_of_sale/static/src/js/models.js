...
models.load_models({
	model: 'res.country.state.region',
...
models.PosModel = models.PosModel.extend({
        get_order_db: function(order){
            return session.rpc('/get_order_by_pos_reference/'+order.name,{}).then(function(order_db) {
                order_db = JSON.parse(order_db);
                self.order_db = order_db;
                //alert(order_db);
                for (var prop in order_db) {
                    if(order_db.hasOwnProperty(prop)) {
                        order[prop] = order_db[prop];
                    }
                }
        ...
        get_order: function(){
            var self = this;
            var order = PosModelSuper.get_order.apply(this, arguments);
            //var order = this.get('selectedOrder');
            //return order;
            if(order == null)
                return order;
            self.get_order_db(order);
            var order_db = self.order_db;
            for (var prop in order_db) {
                if(order_db.hasOwnProperty(prop)) {
                    order[prop] = order_db[prop];
                }
...
models.Order = models.Order.extend({
    ...
    init_from_JSON: function(json) {// carga pedido individual
    	...
    	this.sequence_id = this.pos.get_order().sequence_id;
    	this.sii_document_number = this.pos.get_order().sii_document_number;
